    ...
    <section id="research" className="py-16 md:py-24 border-b border-academic-200 dark:border-academic-800 bg-academic-50/50 dark:bg-academic-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
          ...
          <div className="flex flex-wrap items-center gap-1.5 p-1 rounded-xl bg-academic-100 dark:bg-academic-900 border border-academic-200 dark:border-academic-800 self-start md:self-end">
            {categories.map((cat) => (
              <button
                key={cat}
          ...
          {filteredProjects.map((project) => (
            ...
            >
              <div>
                ...
                {/* Title & Role */}
                <h3 className="text-xl font-serif font-bold text-academic-950 dark:text-white group-hover:text-amber-700 dark:group-hover:text-amber-400 transition-colors">
                  {project.title}
                </h3>
                ...
                <div className="mt-5 flex flex-wrap gap-1.5">
                  {project.techStack.slice(0, 4).map((tech, idx) => (
                    <span
                      key={idx}
                      className="px-2 py-0.5 rounded text-[11px] font-mono bg-academic-100 dark:bg-academic-800/80 text-academic-700 dark:text-academic-300"
                    >
                      {tech}
                    </span>
                  ))}
                  {project.techStack.length > 4 && (
                    <span className="px-1.5 py-0.5 text-[11px] font-mono text-academic-400">
